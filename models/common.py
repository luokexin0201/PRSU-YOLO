...
from Extramodule.ARConv import ARConv
from Extramodule.Scconv import ScConv
from Extramodule.allattention import CombinedAttention, SEAttention, CBAMAttention
...
class PConvAttention(nn.Module):
    ''' Pinwheel-shaped Convolution using the Asymmetric Padding method. '''
    ...
    def __init__(self, c1, c2, k=3, s=2):
        ...
        self.ch = Conv(c1, c2 // 4, (k, 1), s=s, p=0)
        self.cat = Conv(c2, c2, 2, s=1, p=0)
        self.attention = CombinedAttention(c2)
    ...
    def forward(self, x):
        ...
        yh0 = self.ch(self.pad[2](x))
        yh1 = self.ch(self.pad[3](x))
        conv_out = self.cat(torch.cat([yw0, yw1, yh0, yh1], dim=1))
        attntion_out = self.attention(conv_out)
        # print(self.s)
        return attntion_out

class PConvSE(nn.Module):
    ''' Pinwheel-shaped Convolution using the Asymmetric Padding method. '''
...
class PConvCBAM(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=2):
        ...
        self.ch = Conv(c1, c2 // 4, (k, 1), s=s, p=0)
        self.cat = Conv(c2, c2, 2, s=1, p=0)
        self.attention = CBAMAttention(c2)
        self.residual = nn.Conv2d(c1, c2, kernel_size=1, stride=s)
    ...
    def forward(self, x):
        ...
        out = attention_out + residual_out
        # print(self.s)
        return out
...
class RepNCSPELAN4PConv(nn.Module):
    # csp-elan-PCONV
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv4 = Conv(c3+(2*c4)+c4, c2, 1, 1)
        self.ar_branch = PConv2(c3 // 2, c4)

    def forward(self, x):
        y = list(self.cv1(x).chunk(2, 1))
        y.extend((m(y[-1])) for m in [self.cv2, self.cv3, self.ar_branch])
        return self.cv4(torch.cat(y, 1))
...
class RepNCSPELAN4ARConv(nn.Module):
    # csp-elan-ARCONV
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3//2
...
class RepNCSPELAN4ScConv(nn.Module):
    # csp-elan-ScConv
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3//2
