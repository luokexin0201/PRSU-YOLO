...
def inner_siou(box1, box2, xywh=True, eps=1e-7, ratio=0.7):
    ...
    iou = inter / inner_union

    # SIoU
    cw = torch.max(inner_b1_x2, inner_b2_x2) - torch.min(inner_b1_x1, inner_b2_x1)  # convex width
    ch = torch.max(inner_b1_y2, inner_b2_y2) - torch.min(inner_b1_y1, inner_b2_y1)  # convex height
    rho2 = ((inner_b2_x1 + inner_b2_x2 - inner_b1_x1 - inner_b1_x2) ** 2 + (inner_b2_y1 + inner_b2_y2 - inner_b1_y1 - inner_b1_y2) ** 2) / 4  # center distance squared
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, MDPIoU=False, EMACIoU=False, innersiou=False, SDIou=False, feat_h=640, feat_w=640, eps=1e-7, delta = 0.5):
    ...
    if CIoU or DIoU or GIoU or SDIou:
        ...
        if CIoU or DIoU or SDIou:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU or SDIou:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if SDIou:
                    ...
                    # print(1111111)
                    return delta-beta + (1-delta+beta)*(iou-v*alpha) - (1+delta-beta)*(rho2/c2)  # SDIo
        ...
        if ema_loss is None:
            ...
            ema_loss = alpha * ema_loss + (1-alpha)*ciou
        return ema_loss
    elif innersiou:
        # print('111111111111')
        siou = inner_siou(box1=box1, box2=box2, xywh=xywh, eps=eps)
        return siou
    return iou  # IoU
